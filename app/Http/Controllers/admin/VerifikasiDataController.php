...
class VerifikasiDataController extends Controller
{
    public function index()
    {
        ...
        // Add notification for pending data
        $pendingCount = $verifikasi->where('status', 'pending')->count();
        if ($pendingCount > 0) {
            session()->flash('notification', [
                'type' => 'info',
                'title' => 'Data Menunggu Verifikasi',
                'message' => "Terdapat {$pendingCount} data yang menunggu verifikasi Anda"
            ]);
        }

        return view('admin.verifikasi-data.index', compact('verifikasi'));
    }
    ...
    public function approve($id)
    {
        ...
        // Delete verifikasi data after approval (data sudah diproses)
        $verifikasi->delete();
        ...
        session()->flash('notification', [
            ...
            'message' => ucfirst($verifikasi->tabel_terkait) . ' berhasil disetujui dan akan muncul di dashboard siswa'
        ]);
    ...
    public function reject($id)
    {
        ...
        // Delete verifikasi data after rejection (data sudah diproses)
        $verifikasi->delete();
        ...
        session()->flash('notification', [
            'type' => 'warning',
            'title' => 'Data Ditolak',
            'message' => ucfirst($verifikasi->tabel_terkait) . ' telah ditolak dan tidak akan muncul di dashboard siswa'
        ]);
