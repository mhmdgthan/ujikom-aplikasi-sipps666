                    ...
                    <tr data-status="{{ strtolower($item->status_verifikasi) }}">
                        ...
                        <td>
                            <div class="action-buttons">
                                ...
                                <form action="{{ route('guru.pelanggaran.destroy', $item->id) }}" method="POST" class="d-inline form-delete">
                                    @csrf
                                    @method('DELETE')
...
@include('guru.pelanggaran.modals')
@endsection
                            ...
                            ${data.bukti_foto && data.bukti_foto !== '' && data.bukti_foto !== null ? 
                                `<div class="bukti-foto-container">
                                    <img src="/storage/${data.bukti_foto}" 
                                         class="bukti-foto" alt="Bukti Pelanggaran" 
                                         style="max-width: 300px; height: auto; border-radius: 8px; border: 2px solid #e2e8f0; cursor: pointer;" 
                                         onclick="previewFoto('/storage/${data.bukti_foto}')">
                                    <div class="mt-2">
                                        <button class="btn btn-sm btn-primary" onclick="previewFoto('/storage/${data.bukti_foto}')">Preview</button>
                                    </div>
                                </div>` : 
...
document.querySelectorAll('.edit-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        form.action = `{{ url('guru/pelanggaran') }}/${id}`;
        
        fetch(`{{ url('guru/pelanggaran') }}/${id}/edit`)
