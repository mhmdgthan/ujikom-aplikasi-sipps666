...
class LaporanController extends Controller
{
    ...
    public function generate(Request $request)
    {
        ...
        return view('admin.laporan.preview', compact('data', 'request'));
    }
    ...
    private function getData($request)
    {
        ...
        switch ($request->jenis_laporan) {
            case 'pelanggaran':
                $query = Pelanggaran::with(['siswa.user', 'siswa.kelas.jurusan', 'jenisPelanggaran.kategoriPelanggaran', 'tahunAjaran', 'guruPencatat.user'])
        ...
        return $query->get()->map(function($item) use ($request) {
            ...
            switch ($request->jenis_laporan) {
                case 'pelanggaran':
                    ...
                    $item->kategori_pelanggaran_nama = $item->jenisPelanggaran->kategoriPelanggaran->nama_kategori ?? 'Tidak Diketahui';
                ...
                case 'siswa':
                    ...
                    $item->nis = $item->nis ?? 'Tidak Diketahui';
                    $item->nisn = $item->nisn ?? 'Tidak Diketahui';
                    break;
            }
            return $item;
        });
    ...
    {
        try {
            $pdf = PDF::loadView('admin.laporan.pdf', compact('data', 'request'))
            ...
            $callback = function() use ($data, $request) {
                ...
                switch ($request->jenis_laporan) {
                    ...
                    case 'prestasi':
                        ...
                        break;
                    case 'siswa':
                        fputcsv($file, ['No', 'Nama Lengkap', 'NIS', 'NISN', 'Kelas', 'Jurusan', 'Jenis Kelamin', 'Alamat']);
                        foreach ($data as $index => $item) {
                            fputcsv($file, [
                                $index + 1,
                                $item->nama_lengkap,
